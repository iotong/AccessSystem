...
using Common.HttpContextService;
...
namespace Common
{
    public static class Tools
    {
        ...
        {
            HttpHelper.Session.SetObjectAsJson(key, value);
        }
        ...
        public static T GetSession<T>(string key)
        {
            try
            {
                var Obj = HttpHelper.Session.GetObjectFromJson<T>(key);
                if (Obj == null)
        ...
        public static async Task<string> PostAsync(string url, object parameter)
        {
            ...
            byte[] byRemoteInfo = await WebClientObj.UploadValuesTaskAsync(url, "POST", PostVars);
            //这是获取返回信息
            return System.Text.Encoding.UTF8.GetString(byRemoteInfo);
        }
        ...
        {
            if (expires == null)
                HttpHelper.Response.Cookies.Append(key, value);
            else
                HttpHelper.Response.Cookies.Append(key, value, new CookieOptions()
                {
        ...
        {
            return HttpHelper.Request.Cookies[key];
        }
        ...
        {
            HttpHelper.Response.Cookies.Delete(key);
        }
        #endregion Cookie操作
