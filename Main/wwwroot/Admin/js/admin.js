...
var admin = {
    ...
    laytable: null,//表格控件
    init: function () {
        //layui.use(['layer', 'laydate', 'table'], function () { //独立版的layer无需执行这一句
        //    admin.layer = top.layui.layer;
        //    admin.laydate = layui.laydate;
        //    admin.laytable = layui.table;
        //    //layer 扩展皮肤
        //    admin.layer.config({ extend: '../retina.css' });
        //});

        admin.layer = window.top.layui.layer;
        admin.laydate = layui.laydate;
        admin.laytable = layui.table;
        //layer 扩展皮肤
        admin.layer.config({ extend: '../retina.css' });
    ...
    vueDataMapping(_data, _vueObject) {
        //映射数据
        for (let item in _data) {
            var _value = _data[item];
            //如果 vue 模型中 有 该字段
            if (_vueObject.$data.hasOwnProperty(item) && (_value || _value == 0)) {
                if ((typeof _value == 'string') && _value.constructor == String) {//字符串
                    ...
                    if (_value.indexOf('/Date(') >= 0 && _value.lastIndexOf(')/') >= 0)
                        _value = new Date(parseInt(_value.substring(6, value.lastIndexOf(')/')))).Format('yyyy-MM-dd hh:mm');
                    else
                        _value = _value.replace(/\r/g, "\\r").replace(/\n/g, "\\n").replace(/\'/g, "\\'");
                }
                _vueObject.$data[item] = _value;
            }
        }
