...
var adminList = {
    ...
    bootStrapTable: function (options) {
        ...
        $(window).bind('resize', function () {
            adminList.bootStrapTableObj.bootstrapTable('resetView', { height: $(window).height() - 110 });
        });
    ...
    btnControl: function () {
        ...
        if (rows.length > 0) {
            ...
            if (rows.length == 1) {
                adminList.dom.btnEdit().css('display', 'block');//.removeAttr('disabled');
                adminList.dom.btnDelete().css('display', 'block');//.removeAttr('disabled');
            } else {
                adminList.dom.btnEdit().css('display', 'none');//.attr('disabled', true);
                adminList.dom.btnDelete().css('display', 'block');//.removeAttr('disabled');
            }

        } else {
            adminList.dom.btnEdit().css('display', 'none');//.attr('disabled', true);
            adminList.dom.btnDelete().css('display', 'none');//.attr('disabled', true);
        }
    },
    //查询面板
    panelSearch: function () {
        adminList.dom.panelSearch().animate({ height: 'toggle' }, 150);//.toggle();
    },
    //检索
    search: function () {
        adminList.refresh();
